...
<template>
  <slot></slot>
  <div class="">
    <h2><a target="_blank" href="../src/views/AsyncComp.vue">异步组件</a></h2>
    ...
    <p>全量使用：<img src="@/assets/defineAsyncComponent.jpg" alt="defineAsyncComponent" width="400" class="vt" /> 此时就可以将 AsyncComp 变量像普通组件使用即可，<span class="color-orange"><br>
      展示加载组件前有默认200ms延迟的原因是若在网络状况较好时，加载可能很快，若此时还显示加载过渡组件可能会产生闪烁反而影响用户体验，所以设置了该延迟，若在该延迟时间内组件就加载完了也就不用显示加载过渡组件了</span>
    </p>
    ...
    <p>异步组件还可以搭配vue3内置的 <code>&lt;Suspense></code> 组件使用。</p>
  </div>
</template>
