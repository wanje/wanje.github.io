<script setup>
  ...
  // 当一个 ref 值作为 reactive 对象内的属性时(可更深层)，访问对应属性时 ref 会自动解包，但原始 ref 还是要用`.value`访问
  // 这里提到的是对象内，如果是数据或Map这种几个类数据，则不会自动解包，访问时人妖通过`.value`
...
<template>
  <div>
    ...
    <p>该API可用于 <span class="color-orange">所有数据类型</span>，且 <span class="color-orange">具有深层响应性</span>（即用于引用类数据时所有后代属性都具有响应性），
      但<span class="color-red">不能在创建时对其内属性解构赋值，解构值不具有响应性</span>
    </p>
    <p>
