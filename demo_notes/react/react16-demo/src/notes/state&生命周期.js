...
class StateLifeCycle extends React.Component {
  constructor(props){
     ...
     * state中的值不能像props那样直接通过赋值去改变，其数据要使用setState()方法去改变才能触发视图相应改变（类似一个事件通知，而非直接赋值）
     * */
    //* 函数式组件中可以使用 useState hook实现状态管理
    this.state = {
      timer: 0,
      paused: true,
      hasError: false
  ...
  handleClick = () => {
    if (this.state.paused){
      /**
      * 尽管this.props和this.state是react本身设置且具有特殊含义的，
      * 但其实我们可以向class中随意添加不参与数据流的额外字段(也就是this上)，像下面的计时器ID，其不参与DOM及数据流(可以更改其值，只是不用于DOM自动响应更新)
      * */
    ...
    // 传入`setState()`方法的数据不是直接覆盖state属性值，而是与state属性值进行浅合并，所以传入的对象或是函数返回的对象可以只包含当前需要改变的那些状态
    this.setState((preState) => {
      return {
     ...
     * 该函数会以【上一个状态对象state】为第一个参数，【当前更新时将被使用的props对象】为第二个参数，也就是确保这两个数据对象都是最新数据，而不是此次setState前某个异步操作还未完成前的数据
     * */
    // this.setState(function (preState, curProps) {
    //   return {date: preState.counter + curProps.increment}    // 这里只是以计数为例
    // });
  ...
  render() {
    ...
    return this.props.children || (
        <div>
          <span>计时[{this.state.timer}s]</span>
          <button onClick={this.handleClick}>{this.state.paused ? '开始计时' : '暂停计时'}</button>
        </div>
    );
  }
}
